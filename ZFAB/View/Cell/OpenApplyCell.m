...
- (void)setContentsWithData:(TerminalModel *)data {
    _cellData = data;
    self.terminalLabel.text = _cellData.terminalNumber;
...
- (IBAction)openApply:(id)sender {
    if (_delegate && [_delegate respondsToSelector:@selector(openApplyCellOpenWithData:)]) {
        [_delegate openApplyCellOpenWithData:_cellData];
    }
}
