...
- (void)downloadDetail {
    ...
    [NetworkInterface getTerminalDetailWithToken:delegate.token terminalID:_terminalModel.terminalID finished:^(BOOL success, NSData *response) {
...
- (void)synchronizeTerminal {
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
    hud.labelText = @"加载中...";
    [NetworkInterface synchronousTerminalWithTerminalID:_terminalModel.terminalID finished:^(BOOL success, NSData *response) {
        NSLog(@"%@",[[NSString alloc] initWithData:response encoding:NSUTF8StringEncoding]);
        hud.customView = [[UIImageView alloc] init];
        hud.mode = MBProgressHUDModeCustomView;
...
- (void)findPOSPassword {
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
    hud.labelText = @"加载中...";
    [NetworkInterface findPOSPasswordWithTerminalID:_terminalModel.terminalID finished:^(BOOL success, NSData *response) {
        NSLog(@"%@",[[NSString alloc] initWithData:response encoding:NSUTF8StringEncoding]);
        hud.customView = [[UIImageView alloc] init];
        hud.mode = MBProgressHUDModeCustomView;
        [hud hide:YES afterDelay:0.5f];
...
- (void)addButton {
    //已开通
    if (_terminalModel.status == TerminalStatusOpened) {
    ...
    else if (_terminalModel.status == TerminalStatusPartOpened) {
        //部分开通
    ...
    else if (_terminalModel.status == TerminalStatusCanceled) {
        //已注销
        UIButton *openConfirmBtn = [self buttonWithTitle:@"重新申请开通" action:@selector(openConfirm:)];
        UIButton *videoAuthBtn = [self buttonWithTitle:@"视频认证" action:@selector(videoAuth:)];
        if (_terminalModel.appID && ![_terminalModel.appID isEqualToString:@""]) {
